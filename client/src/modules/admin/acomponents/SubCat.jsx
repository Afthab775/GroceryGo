...
export default function SubCat() {
  ...
  const validateForm = () => {
    ...
    if (!subcategory.subimage) {
      showSnackbar("Please upload a subcategory image", "warning");
      return false;
    }
    return true;
  };
  ...
  const handleinsert = async () => {
    ...
    try {
      ...
      data.append('subname', subcategory.subname);
      data.append('subimage', subcategory.subimage);
      data.append('cid', subcategory.cid);

      const res = await axios.post(`${import.meta.env.VITE_API_URL}/api/subcategory/addsubcategory`, data, {
        headers: {
      ...
      if (error.response?.status === 401 || error.response?.status === 403) {
        ...
        showSnackbar(error.response?.data?.message || "Failed to add subcategory", "error");
      }
      setLoading(false);
    }
  };
        ...
        <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 1.5 }}>
          ...
          <Box>
            ...
            <Typography variant="caption" sx={{ color: '#666', lineHeight: 1.6 }}>
              Subcategories help organize products within a category. For example, under "Fruits & Vegetables",
              you can have subcategories like "Fresh Fruits", "Exotic Fruits", or "Organic Vegetables".
            </Typography>
          </Box>
        </Box>
      </Paper>
